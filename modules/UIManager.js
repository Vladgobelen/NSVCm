...
class UIManager {
    ...
    static addMessage(user, text, timestamp = null, type = 'text', imageUrl = null, messageId = null, readBy = [], userId = null, broadcast = false, thumbnailUrl = null, targetContainer = null) {
    ...
    static prependMessageToContainer(user, text, timestamp = null, type = 'text', imageUrl = null, messageId = null, readBy = [], userId = null, broadcast = false, thumbnailUrl = null, container = null) {
    ...
    static clearContainerMessages(container = null) {
        ...
        if (sentinel && targetContainer.firstChild !== sentinel) {
    ...
    static prependMessagesBatch(messages) {
        ...
        for (let i = 0; i < messages.length; i++) {
            ...
            const el = this._createMessageElement(msg.username, msg.text, msg.timestamp, msg.type, msg.imageUrl, msg.id, msg.readBy || [], msg.userId, false, msg.thumbnailUrl);
            if (el) {
    ...
    static _createMemberElement(user, savedSliderValues, isOnline) {
        ...
        if (isOnline) {
            ...
            memberElement.addEventListener('mouseleave', () => {
                setTimeout(() => { if (!slider.matches(':hover')) slider.style.display = 'none'; }, 100);
            });
    ...
    static showError(message) {
        ...
        setTimeout(() => {
            if (document.body.contains(errorElement)) document.body.removeChild(errorElement);
        }, 5000);
    }
        ...
        if (this.secondaryChat.enabled) {
            ...
            secondaryFrame.innerHTML = `
                <div class="secondary-chat-header">
                    <select class="secondary-room-selector"><option value="">📋 Выберите комнату...</option></select>
                    <button class="secondary-close-btn" title="Закрыть">✕</button>
                </div>
                <div class="secondary-room-list"><div class="no-results">Загрузка комнат...</div></div>
            ...
            if (client.secondaryChat.roomId) {
    ...
    static async openSecondaryChatDirect(client, roomId) {
        ...
        if (this.secondaryChat.enabled && this.secondaryChat.roomId === roomId) {
            return;
        }
        if (!this.secondaryChat.enabled) {
            await client.toggleSecondaryChat('side');
        }
        setTimeout(async () => {
            if (this.secondaryChat.enabled) {
                await this._joinSecondaryRoom(client, roomId);
            }
        }, 200);
    }
        ...
        if (roomList) {
            roomList.addEventListener('click', async (e) => {
                ...
                if (item && item.dataset.roomId && !this.secondaryChat.isLoading) {
                    await this._joinSecondaryRoom(client, item.dataset.roomId);
                }
            });
        }
        if (inputEl) {
            ...
            const handleSend = async () => {
                ...
                if (text && client) {
                    await client.sendSecondaryMessage(text, this.secondaryChat.roomId);
                    inputEl.value = '';
    ...
    static async _loadSecondaryRoomOptions(client) {
        ...
        roomSelector.innerHTML = '<option value="">📋 Выберите комнату...</option>';
        try {
            ...
            for (const server of servers) {
                ...
                const serverName = this.escapeHtml(isPrivateServer
                    ? `👤 ${server.displayName || server.name || 'Приватный'}`
                    : `🏠 ${server.name}`);
                ...
                if (isPrivateServer) {
                    ...
                    for (const room of serverRooms) {
                        ...
                        const roomName = this.escapeHtml(room.name || room.id || 'Комната');
            ...
            roomList.innerHTML = listHtml || '<div class="no-results">Нет доступных комнат</div>';
        } catch (error) {
    ...
    static async _joinSecondaryRoom(client, roomId) {
        ...
        if (client.socket?.connected) {
            client.socket.emit('secondary-chat-update', { roomId: roomId });
        }
    ...
    static addSecondaryMessage(user, text, timestamp = null, type = 'text', imageUrl = null, messageId = null, readBy = [], userId = null, broadcast = false, thumbnailUrl = null) {
        if (!this.secondaryChat.enabled || !this.secondaryChat.messagesContainer) return;
        if (messageId && this.secondaryChat.messagesContainer.querySelector(`[data-message-id="${messageId}"]`)) {
            return;
        }
        ...
        this.addMessage(
            user, text, timestamp, type, imageUrl, messageId, readBy, userId, broadcast, thumbnailUrl,
            this.secondaryChat.messagesContainer
        );
    }
}
