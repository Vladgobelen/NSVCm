...
class MediaManager {
  static async connect(client, roomId, mediaData) {
    try {
      ...
      // 🔥 ВАЖНО: Вызываем produce СРАЗУ. Не ждем ICE. Пример работает именно так.
  ...
  static setupTransportStateChangeHandler(client, transport) {
    transport.on('connectionstatechange', (state) => {
      ...
      if (state === 'failed' || state === 'disconnected') {
        console.warn(`⚠️ [MEDIA] ICE-соединение транспорта ${transport.id} упало (state: ${state}).`);
  ...
  static setupTransportConnectHandler(client, transport) {
    ...
    transport.on('connect', async ({ dtlsParameters }, callback, errback) => {
      // 🔥 Флаг как в рабочем примере: предотвращает двойной вызов
      ...
      client.socket.emit('transport-connect', {
        ...
        if (response?.success) {
          transport.__connected = true; // 🔥 Ставим флаг
  ...
  static async initMicrophone(client) {
    try {
      ...
      track.enabled = false; // Сразу глушим, пока не нажмут кнопку

      // 🔥 Вызываем produce СРАЗУ. Это запустит ICE/DTLS асинхронно.
      ...
      console.error('❌ [MIC] Failed to init microphone:', error.message);
  ...
  // 🔥 Упрощено как в рабочем примере. Никаких проверок транспорта.
  ...
  static async createConsumer(client, consumerParams) {
    ...
    // 🔥 ИСПРАВЛЕНО: Добавляем элемент в DOM, иначе мобильные браузеры блокируют autoplay
    const audioElement = document.createElement('audio');
    audioElement.id = `audio-${consumerParams.producerId}`;
    audioElement.autoplay = true;
    audioElement.playsInline = true; // 🔥 Критично для iOS/Android
    ...
    // 🔥 Обрабатываем блокировку автовоспроизведения
    const playPromise = audioElement.play();
    if (playPromise !== undefined) {
      playPromise.catch(error => {
        console.warn(`⚠️ [AUDIO] Autoplay blocked for ${consumerParams.producerId}: ${error.name}`);
