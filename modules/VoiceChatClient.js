...
class VoiceChatClient {
    constructor() {
        ...
        this.socketRoom = null; // ✅ НОВОЕ: Отслеживаем комнату сокета
        this.init();
    }
    ...
    async joinRoom(roomId) {
        ...
        try {
            ...
            // ✅ ИСПРАВЛЕНО: Выход из старой комнаты перед входом в новую
            ...
            // ✅ ИСПРАВЛЕНО: Пересоздаем сокет для новой комнаты
    ...
    setupSocketConnection() {
        ...
        // ✅ ПРОВЕРКА: Если сокет существует, но для другой комнаты - пересоздаем
        ...
        try {
            ...
            // ✅ ИСПРАВЛЕНО: Авто-переподключение при разрыве
            socket.on('disconnect', (reason) => {
                console.log('Socket disconnected:', reason);
                UIManager.updateStatus('Отключено', 'disconnected');
                
                // ✅ АВТО-ПЕРЕПОДКЛЮЧЕНИЕ: Если разрыв не по инициативе клиента
