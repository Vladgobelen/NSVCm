...
class MessageRenderer {
    ...
    static _updateAllReplyOrientations() {
        ...
        const groups = document.querySelectorAll('.message-reply-group');
    ...
    static _createMessageElement(user, text, timestamp = null, type = 'text', imageUrl = null, messageId = null, readBy = [], userId = null, broadcast = false, thumbnailUrl = null, replyTo = null, reactions = {}, poll = null, forwardedFrom = null, pollRef = null, embed = null) {
        const self = this;
        ...
        messageEl.className = `message ${type === 'system' ? 'system-message' : ''} ${type === 'poll' ? 'poll-message' : ''}`;
        ...
        if (isDirectedToMe) {
            ...
            if (directedType === 'reply') {
                messageEl.classList.add('message-reply-to-me');
            } else if (directedType === 'mention') {
                messageEl.classList.add('message-mention-me');
            } else if (directedType === 'name_mention') {
                messageEl.classList.add('message-name-mention-me');
            }
        }
        
        if (embed) {
            messageEl.dataset.embed = JSON.stringify(embed);
        }
        
        if (poll) {
            const pollDataForDataset = { poll, messageId, pollRef };
            if (client?.currentRoom) {
                pollDataForDataset.roomId = client.currentRoom;
            }
            messageEl.dataset.pollData = JSON.stringify(pollDataForDataset);
        }
        if (pollRef) {
            messageEl.dataset.pollRef = JSON.stringify(pollRef);
        }
        if (forwardedFrom) {
            messageEl.dataset.forwardedFrom = JSON.stringify(forwardedFrom);
        }
        ...
        messageEl.addEventListener('contextmenu', (event) => {
            ...
            if (messageId && userId) {
                ...
                if (forwardedFrom) {
                    ContextMenuManager.showForwardedMessageContextMenu(event, messageId, msgObj);
                } else {
                    ContextMenuManager.showMessageContextMenu(event, messageId, userId, safeUser, timestamp, msgObj);
                }
            }
        });
        ...
        messageEl.addEventListener('click', (event) => {
            ...
            const blockedSelector = 'a, button, input, textarea, .reply-block, .message-context-menu, .reaction-pill, .reaction-picker-inline, .forwarded-badge, .poll-option, .poll-vote-btn, .embed-thumbnail, .embed-title';
            const blockedTarget = event.target.closest(blockedSelector);
            if (blockedTarget) return;

            const textToCopy = safeText;
        ...
        const headerHtml = `<div class="message-header">
            ...
            ${type !== 'poll' ? '<button class="message-reply-btn" title="Ответить">↩️</button>' : ''}
        </div>`;
        
        let contentBodyHtml = '';
        if (type === 'image') {
            const displayUrl = finalThumbnailUrl || finalImageUrl;
            contentBodyHtml = `<div class="image-thumbnail" data-full-size="${finalImageUrl.replace(/"/g, '&quot;')}" style="cursor:pointer;"><img src="${displayUrl.replace(/"/g, '&quot;')}" alt="Изображение" loading="lazy"><div class="image-overlay">🔍</div></div>`;
        } else if (type === 'poll') {
            const pollDataForContainer = { poll, messageId, pollRef };
            if (client?.currentRoom) {
                pollDataForContainer.roomId = client.currentRoom;
            }
        ...
        if (forwardedBadge) {
            forwardedBadge.addEventListener('click', (e) => {
                ...
                if (client && typeof client.jumpToForwardSource === 'function') {
                    client.jumpToForwardSource(forwardedFrom);
                }
            });
        }
        ...
        if (replyBtn) {
            const msgObj = { id: messageId, userId, username: safeUser, text: safeText, timestamp, type, imageUrl, thumbnailUrl, poll, pollRef, embed };
    ...
    static async addMessage(user, text, timestamp = null, type = 'text', imageUrl = null, messageId = null, readBy = [], userId = null, broadcast = false, thumbnailUrl = null, targetContainer = null, replyTo = null, reactions = {}, poll = null, forwardedFrom = null, pollRef = null, embed = null) {
        ...
        const messageElement = this._createMessageElement(user, text, timestamp, type, imageUrl, messageId, readBy, userId, broadcast, thumbnailUrl, replyTo, reactions, poll, forwardedFrom, pollRef, embed);
    ...
    static async prependMessage(user, text, timestamp = null, type = 'text', imageUrl = null, messageId = null, readBy = [], userId = null, broadcast = false, thumbnailUrl = null, replyTo = null, reactions = {}, poll = null, forwardedFrom = null, pollRef = null, embed = null) {
        const container = document.querySelector('.messages-container');
        if (!container) return;
    ...
    static _renderEmbed(embed) {
        ...
        if (embed.provider === 'direct_media' && embed.image) {
            ...
            <div class="message-embed embed-image-only">
                <div class="embed-image-container" data-full-image="${this.escapeHtml(fullImageUrl)}">
                    <img src="${this.escapeHtml(imageUrl)}" alt="Изображение" loading="lazy" style="max-width:100%; max-height:400px; border-radius:8px; cursor:pointer;">
                </div>
            </div>`;
        }
        ...
        if (embed.imageData && embed.imageData.thumbnailPath) {
            ...
            <div class="embed-thumbnail" data-full-image="${this.escapeHtml(fullImageUrl)}">
                <img src="${this.escapeHtml(imageUrl)}" alt="${title}" loading="lazy">
            </div>`;
        } else if (embed.image) {
            imageHtml = `
            <div class="embed-thumbnail">
                <img src="${this.escapeHtml(embed.image)}" alt="${title}" loading="lazy" referrerpolicy="no-referrer">
            </div>`;
        }
        
        let faviconHtml = '';
        if (embed.favicon) {
            faviconHtml = `<img class="embed-favicon" src="${this.escapeHtml(embed.favicon)}" alt="" loading="lazy">`;
        }
    ...
    static updateMessageEmbed(messageId, embed) {
        ...
        if (reactionsBlock) {
            contentContainer.insertBefore(embedElement, reactionsBlock);
        } else {
            contentContainer.appendChild(embedElement);
        }
