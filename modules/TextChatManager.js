...
class TextChatManager {
                        ...
                        if (beforeId && container) {
                            ...
                            for (const msg of response.messages) {
                                const el = UIManager._createMessageElement(
                                    ...
                                    msg.imageUrl, msg.id, msg.readBy || [], msg.userId, false, msg.thumbnailUrl, msg.replyTo
                                );
                                if (el) {
                            ...
                            for (const msg of response.messages) {
                                UIManager.addMessage(
                                    msg.username, msg.text, msg.timestamp, msg.type,
                                    msg.imageUrl, msg.id, msg.readBy || [], msg.userId, false, msg.thumbnailUrl, container, msg.replyTo
                                );
                            }
                        }
                    }
    ...
    static loadMessagesAround(client, roomId, messageId, limit = 50, targetContainer = null) {
        return new Promise((resolve, reject) => {
            ...
            client.socket.emit('request-message-history', { roomId, limit, aroundId: messageId }, (response) => {
                if (response?.success && Array.isArray(response.messages)) {
                    if (targetContainer) {
                        UIManager.clearContainerMessages(targetContainer);
                    }
                    if (response.messages.length > 0) {
                        const container = targetContainer || document.querySelector('.messages-container');
                        if (container) {
    ...
    static async uploadImage(client, roomId, file) {
        ...
        try {
            ...
            console.error('Ошибка загрузки изображения:', fetchError);
            throw fetchError;
        }
    }
    ...
    static async sendMessage(client, content, type = 'text', replyTo = null) {
        ...
        if (type === 'text' && content && content.trim() === '-отладка') {
            if (client.socket) client.socket.emit('send-message', { roomId: client.currentRoom, text: content.trim(), replyTo });
            return;
        }
        let payload;
        if (type === 'text') {
