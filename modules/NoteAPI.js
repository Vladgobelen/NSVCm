...
class NoteAPI {
  ...
  async deleteNote(type = 'personal', noteId, targetId = null) {
    ...
    return this._fetch(`/${noteId}?${params.toString()}`, {
      method: 'DELETE'
    });
  }
  ...
  async loadThread(noteId, limit = 50, beforeId = null) {
    ...
    // ИСПРАВЛЕНО: сервер возвращает поле 'notes', но клиент ожидал 'messages'.
    // Теперь проверяем оба варианта для совместимости.
    return {
