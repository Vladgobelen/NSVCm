...
class NoteUIManager {
  ...
  static init(client, api) {
    this.client = client;
    this.api = api;
    // 🔥 КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: передаем клиента в рендерер, чтобы работал isOwn
    NoteRenderer.setClient(client);
  ...
  static _getRoomName(roomId) {
    ...
    try {
      ...
      // Игнорируем ошибки доступа к кэшу
    }
    return roomId;
  }
  ...
  static _setupEventListeners() {
    ...
    const closeBtnCentral = document.querySelector('.notes-close-btn');
    const closeBtnSidebar = document.querySelector('.notes-list-close-btn');
    ...
    const handleClose = () => this.returnToChat();
    if (closeBtnCentral) closeBtnCentral.addEventListener('click', handleClose);
    if (closeBtnSidebar) closeBtnSidebar.addEventListener('click', handleClose);

    if (threadBackBtn) threadBackBtn.addEventListener('click', () => this.returnToChat());

    // 🔥 ИСПРАВЛЕННАЯ ЛОГИКА ОТПРАВКИ В ТРЕД
    if (threadInput && threadSendBtn) {
      const sendThread = async () => {
        const text = threadInput.value.trim();
        const context = NoteStateManager.context;
        if (!text || !context.noteId) return;

        // Для публичных заметок targetId = roomId. Для личных берем текущую комнату клиента.
        const roomId = this.client?.currentRoom || context.targetId;
        if (!roomId) {
          console.warn('Не удалось определить комнату для отправки в тред');
          return;
        }
        ...
        try {
          ...
          await this.api.sendThreadMessage(context.noteId, roomId, text);
          this.loadThread(context.noteId);
        } catch (error) {
          console.error('Критическая ошибка отправки в тред:', error);
          alert('Не удалось отправить ответ: ' + (error.message || 'Ошибка сети'));
        }
      };
    ...
    if (notesContainer && !document.getElementById('new-note-btn')) {
      ...
      else notesContainer.prepend(btn);
    }
  ...
  static _updateHeader(mode) {
    ...
    if (!title) return;
    if (mode === 'personal') title.textContent = '📝 Личные заметки';
    else if (mode === 'public') {
    ...
    const save = async () => {
      ...
      if (text) {
        ...
        try {
          ...
          console.error('Критическая ошибка создания заметки:', error);
        }
      } else {
        editor.remove();
      }
  ...
  static async loadNotes(type, targetId, isInitial) {
    ...
    try {
      ...
      if (isInitial) {
        ...
        // 🔥 Загружаем имена авторов перед рендерингом
        this._fetchNoteAuthors(notes);
      ...
      console.error('Критическая ошибка загрузки заметок:', error);
  ...
  // 🔥 Метод для сбора и подгрузки ID авторов
  static _fetchNoteAuthors(notes) {
    if (!notes || notes.length === 0) return;
    const missingIds = new Set();
    for (const note of notes) {
      if (note.authorId && !UIManager.usernameCache.has(note.authorId)) {
        missingIds.add(note.authorId);
      }
    }
    if (missingIds.size > 0) {
      UIManager.fetchUsernames(Array.from(missingIds));
    }
  }
  ...
  static renderSidebarList(notes) {
    ...
    const fragment = document.createDocumentFragment();
    notes.forEach(note => {
      fragment.appendChild(NoteRenderer.createSidebarItem(note, 'sidebar', (id) => this.scrollToNote(id)));
    });
    container.innerHTML = '';
    container.appendChild(fragment);
  }
  ...
  static _setupInfiniteScroll() {
    ...
    this.currentScrollObserver = new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting && !this.loading) {
        ...
        if (state.hasMore) this.loadNotes(type, context.targetId, false);
      }
  ...
  static handleEdit(noteId, cardElement) {
    ...
    if (note) {
      NoteRenderer.enableInlineEditor(cardElement, note, type, context.targetId, async (id, text) => {
        try {
          await this.api.updateNote(type, id, text, context.targetId);
          this.loadNotes(type, context.targetId, true);
        } catch (error) {
          console.error('Критическая ошибка обновления заметки:', error);
        }
      }, () => {});
    }
  }
  ...
  static async handleDelete(noteId, type, targetId) {
    ...
    try {
      ...
      console.error('Критическая ошибка удаления заметки:', error);
    }
  }

  static async handleReact(noteId, emoji, type, targetId) {
    // Реализация реакции будет добавлена при необходимости
  }
  ...
  static async loadThread(noteId) {
    ...
    try {
      ...
      // 🔥 Подгрузка имен для сообщений треда
      this._fetchThreadAuthors(messages);
      ...
      console.error('Критическая ошибка загрузки треда:', error);
    }
  }

  // 🔥 Метод для треда
  static _fetchThreadAuthors(messages) {
      if (!messages || messages.length === 0) return;
      const missingIds = new Set();
      for (const msg of messages) {
        if (msg.userId && !UIManager.usernameCache.has(msg.userId)) {
